...
void ShaderUniformDemo::add_shader(GLuint shaderProgram, GLuint *shaderObject, const char *pShaderText, GLenum shaderType)
{
...
void ShaderUniformDemo::compile_shaders(GLuint *shaderProgram, GLuint *vertexShaderObject, GLuint *fragmentShaderObject)
{
    *shaderProgram = glCreateProgram();
    ...
    m_scaleLocation = glGetUniformLocation(*shaderProgram, "gScale");
    if (m_scaleLocation == -1)
    {
...
ShaderUniformDemo *ShaderUniformDemo::s_instance = NULL;

static void init(void)
{
    ShaderUniformDemo::Instance()->Init();
}

static void render(void)
{
    ShaderUniformDemo::Instance()->Render();
}

ShaderUniformDemo::ShaderUniformDemo()
{
    Engine::Instance()->RegisterModuleInitCallback(init);
    GraphicsEngine::Instance()->RegisterRenderCallback(render);
}

ShaderUniformDemo *ShaderUniformDemo::Instance(void)
{
    if (s_instance == NULL)
    {
        s_instance = new ShaderUniformDemo();
    }

    return s_instance;
}

void ShaderUniformDemo::Init(void)
{
    m_scale = 1.0f;
...
void ShaderUniformDemo::Render(void)
{
    m_scale -= 0.1f;
    if (m_scale <= 0)
    {
    ...
    glUniform1f(m_scaleLocation, m_scale);
